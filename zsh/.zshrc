...
export PATH="$BUN_INSTALL/bin:/usr/local/mysql/bin:$PATH"
...
# JAVA_HOME — JDK 17 for Gradle/RN (set only if a 17 runtime exists)
if /usr/libexec/java_home -v 17 &>/dev/null; then export JAVA_HOME="$(/usr/libexec/java_home -v 17)"; fi

# Aion service-account token: NOT stored here. Load on demand when needed:
#   source aion-op-token        (sets $OP_SERVICE_ACCOUNT_TOKEN; ~/bin/aion-op-token)
